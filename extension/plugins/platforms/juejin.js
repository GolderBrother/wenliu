...
class JuejinPlatformPlugin extends BasePlatformPlugin {
  ...
  async postFillProcess(elements, data, results) {
    ...
    // 发送掘金特有的事件
    wenliuEventBus.emit('juejin:fillComplete', {
      results,
      mode: 'title_only'
    });
...
if (window.wenliuPlatformRegistry && window.wenliuPluginConfig) {
  const juejinConfig = window.wenliuPluginConfig.platforms.find(p => p.id === 'juejin');
  ...
  if (juejinConfig && juejinConfig.enabled) {
    ...
    if (shouldRegister) {
      ...
      wenliuPlatformRegistry.register(juejinPlugin);
    }
  }
}
