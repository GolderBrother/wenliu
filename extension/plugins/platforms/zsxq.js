...
class ZsxqPlatformPlugin extends BasePlatformPlugin {
  ...
  async postFillProcess(elements, data, results) {
    ...
    // 发送知识星球特有的事件
    wenliuEventBus.emit('zsxq:fillComplete', {
      results,
  ...
  createGroupSelectorDialog(groups, allowMultiple, onComplete) {
    ...
    overlay.className = 'wenliu-group-selector-overlay';
    ...
    dialog.className = 'wenliu-group-selector-dialog';
    ...
    // 添加现代化CSS动画
    if (!document.querySelector('#wenliu-group-selector-styles')) {
      const styles = document.createElement('style');
      styles.id = 'wenliu-group-selector-styles';
      styles.textContent = `
        @keyframes slideInUp {
          from {
            opacity: 0;
        ...
        .wenliu-group-item:hover {
          background: #f8fafc !important;
          border-color: #3b82f6 !important;
          transform: translateY(-2px) !important;
          box-shadow: 0 4px 20px rgba(59, 130, 246, 0.15) !important;
        }
        .wenliu-group-item.selected {
        ...
        .wenliu-group-list::-webkit-scrollbar {
          width: 6px;
        }
        .wenliu-group-list::-webkit-scrollbar-track {
          background: transparent;
        }
        .wenliu-group-list::-webkit-scrollbar-thumb {
          background: #e2e8f0;
          border-radius: 3px;
        }
        .wenliu-group-list::-webkit-scrollbar-thumb:hover {
          background: #cbd5e1;
        }
        #wenliu-group-cancel:hover {
          background: #f8fafc !important;
          border-color: #cbd5e1 !important;
          color: #475569 !important;
        }
        #wenliu-group-confirm:hover {
          transform: translateY(-1px) !important;
          box-shadow: 0 6px 20px rgba(59, 130, 246, 0.4) !important;
        }
        #wenliu-select-all:hover {
          border-color: #cbd5e1 !important;
    ...
    dialog.innerHTML = `
      <div style="display: flex; align-items: center; margin-bottom: 20px;">
        ...
        ${allowMultiple ? `
          <button id="wenliu-select-all" style="
      ...
      ">
        ...
        <div style="display: flex; gap: 8px;">
          <button id="wenliu-group-cancel" style="
          ...
          ${allowMultiple ? `
            <button id="wenliu-group-confirm" style="
    ...
    const updateSelectAllButton = () => {
      const selectAllBtn = dialog.querySelector('#wenliu-select-all');
    ...
    dialog.addEventListener('click', (e) => {
      const groupItem = e.target.closest('.wenliu-group-item');
      const checkbox = e.target.type === 'checkbox' ? e.target : null;
      
      // 全选按钮
      if (e.target.id === 'wenliu-select-all') {
        e.stopPropagation();
        const allSelected = selectedGroups.size === groups.length;
        ...
        if (allSelected) {
          ...
          dialog.querySelectorAll('input[type="checkbox"]').forEach(cb => {
            cb.checked = false;
            const groupItem = cb.closest('.wenliu-group-item');
            if (groupItem) {
      ...
      // 取消按钮
      if (e.target.id === 'wenliu-group-cancel') {
        overlay.remove();
        onComplete(null);
      }
      
      // 确认按钮（多选模式）
      if (e.target.id === 'wenliu-group-confirm') {
  ...
  async publishToGroup(data, group) {
    try {
      ...
      // 添加预设开头内容
      const currentPreset = window.wenliuApp?.getSelectedPreset?.();
  ...
  createPublishStatusDialog(selectedGroups) {
    const overlay = document.createElement('div');
    overlay.className = 'wenliu-publish-status-overlay';
    overlay.style.cssText = `
      position: fixed; top: 0; left: 0; width: 100%; height: 100%;
    ...
    const dialog = document.createElement('div');
    dialog.className = 'wenliu-publish-status-dialog';
    dialog.style.cssText = `
...
// 配置驱动的自动注册
if (window.wenliuPlatformRegistry && window.wenliuPluginConfig) {
  const zsxqConfig = window.wenliuPluginConfig.platforms.find(p => p.id === 'zsxq');
  ...
  if (zsxqConfig && zsxqConfig.enabled) {
    ...
    if (shouldRegister) {
      ...
      wenliuPlatformRegistry.register(zsxqPlugin);
    }
  }
}
