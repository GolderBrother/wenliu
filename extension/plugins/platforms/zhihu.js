...
class ZhihuPlatformPlugin extends BasePlatformPlugin {
  ...
  _findElements() {
    ...
    if (isZhihuEditor) {
      ...
      elements.elements.title = sortedElements.find(element =>
        !element.id.includes('wenliu') &&
        !element.className.includes('wenliu') &&
  ...
  async fillContent(data) {
    ...
    wenliuEventBus.emit('platform:fillComplete', { 
      platform: this.id, 
      results,
      data,
      mode: 'title_only'
  ...
  async postFillProcess(elements, data, results) {
    ...
    // 发送知乎特有的事件
    wenliuEventBus.emit('zhihu:fillComplete', {
      results,
...
if (window.wenliuPlatformRegistry && window.wenliuPluginConfig) {
  const zhihuConfig = window.wenliuPluginConfig.platforms.find(p => p.id === 'zhihu');
  ...
  if (zhihuConfig && zhihuConfig.enabled) {
    ...
    if (shouldRegister) {
      ...
      wenliuPlatformRegistry.register(zhihuPlugin);
    }
  }
}
