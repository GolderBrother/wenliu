...
class BasePlatformPlugin {
  ...
  async init() {
    ...
    wenliuEventBus.emit('platform:init', { id: this.id });
  }
  ...
  async destroy() {
    ...
    wenliuEventBus.emit('platform:destroy', { id: this.id });
  }
  ...
  async copyArticleContent(articleId) {
    try {
      ...
      // 获取文章内容
      const response = await window.wenliuApiService.articles.get(articleId, 'raw');
      if (!response.success) {
