/**
 * 新架构 - 配置服务
 * 替代旧的wenliuConstants，提供统一的配置管理服务
 */
class ConfigService {
  constructor() {
    this.config = {
      ...
      // UI配置
      PANEL_ID: 'wenliu-assistant-panel',
  ...
  async init() {
    ...
    try {
      ...
      // 触发配置加载完成事件
      if (window.wenliuEventBus) {
        wenliuEventBus.emit('config:loaded', this.config);
      }
    } catch (error) {
  ...
  loadPlatformConfigs() {
    ...
    window.wenliuPluginConfig.platforms.forEach(platform => {
      if (platform.enabled) {
        // 提取主域名
  ...
  set(key, value) {
    ...
    // 触发配置变更事件
    if (window.wenliuEventBus) {
      wenliuEventBus.emit('config:changed', { key, value });
    }
  }
  ...
  async setApiBaseUrl(url) {
    ...
    try {
      ...
      // 触发URL变更事件
      if (window.wenliuEventBus) {
        wenliuEventBus.emit('config:apiUrlChanged', url);
      }
    } catch (error) {
  ...
  merge(newConfig) {
    ...
    // 触发配置合并事件
    if (window.wenliuEventBus) {
      wenliuEventBus.emit('config:merged', newConfig);
    }
  }
  ...
  reset() {
    ...
    // 触发重置事件
    if (window.wenliuEventBus) {
      wenliuEventBus.emit('config:reset');
    }
  }
...
// 创建全局实例
window.wenliuConfigService = new ConfigService();

console.log('✅ 文流配置服务已加载');