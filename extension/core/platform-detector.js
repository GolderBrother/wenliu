...
class wenliuPlatformDetector {
  ...
  getPlatformEditorUrl(platformId) {
    if (!window.wenliuPluginConfig?.platforms) {
      return null;
    }

    const platform = window.wenliuPluginConfig.platforms.find(p => p.id === platformId);
  ...
  getSupportedPlatforms() {
    ...
    return window.wenliuPluginConfig.platforms
      .filter(p => p.enabled)
      .map(p => ({
        id: p.id,
        name: p.displayName,
...
// 创建全局实例
window.wenliuPlatformDetector = new wenliuPlatformDetector();
