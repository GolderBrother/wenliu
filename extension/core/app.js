...
class wenliuApp {
  ...
  async init() {
    if (this.isInitialized) {
      console.log('⚠️ 文流助手已初始化');
      return;
    }

    console.log('🚀 文流助手开始初始化...');
    ...
    try {
      ...
      wenliuEventBus.emit('app:ready', { 
        platform: this.currentPlatform?.id,
        timestamp: Date.now()
      });

    } catch (error) {
      console.error('❌ 文流助手初始化失败:', error);
      wenliuEventBus.emit('app:error', { error });
      throw error;
    }
  }
  ...
  async loadConfig() {
    ...
    this.config = window.wenliuPluginConfig;
  ...
  async loadPlatformPlugin(config) {
    ...
    // 首先检查是否已经有专用插件注册
    const existingPlatform = wenliuPlatformRegistry.get(id);
    if (existingPlatform) {
      console.log(`✅ 使用已注册的专用插件: ${config.displayName}`);
      const instance = wenliuPlatformRegistry.getInstance(id);
      if (instance) {
        await instance.init();
      }
      return instance;
    ...
    wenliuPlatformRegistry.register(instance);
  ...
  async initUI() {
    ...
    try {
      ...
      if (window.wenliuPanel) {
        window.wenliuPanel.init();
        console.log('✅ UI面板已创建');
      } else {
        console.warn('⚠️ wenliuPanel 未找到，跳过UI初始化');
      }

      // 初始化功能管理器
      if (window.wenliuFeatures) {
        window.wenliuFeatures.init();
        console.log('✅ 功能管理器已初始化');
      } else {
        console.warn('⚠️ wenliuFeatures 未找到，跳过功能初始化');
      }
      
      wenliuEventBus.emit('app:uiReady');
      console.log('✅ UI组件初始化完成');
    } catch (error) {
      console.error('❌ UI初始化失败:', error);
      wenliuEventBus.emit('app:uiReady'); // 即使失败也发送事件
    }
  }
  ...
  async checkAuth() {
    ...
    wenliuEventBus.emit('app:authReady');
  }
  ...
  async loadPresets() {
    ...
    try {
      // 使用新的ApiService获取预设
      const response = await wenliuApiService.presets.list();
      ...
      if (response.success && Array.isArray(response.data)) {
        ...
        // 通知UI更新
        wenliuEventBus.emit('presets:loaded', { 
  ...
  setSelectedPreset(presetId) {
    if (presetId === null || presetId === 'none') {
      ...
      wenliuEventBus.emit('presets:changed', { selectedPreset: null });
      return;
    }
    ...
    if (preset) {
      ...
      wenliuEventBus.emit('presets:changed', { selectedPreset: preset });
    } else {
  ...
  async loadUserData() {
    ...
    wenliuEventBus.emit('app:dataReady');
  }
  ...
  getCurrentPlatform() {
    ...
    return wenliuPlatformRegistry.getInstance(this.currentPlatform.id);
  }
  ...
  setState(key, value) {
    ...
    wenliuEventBus.emit('state:change', { key, value });
  }
   ...
   */
  async destroy() {
    console.log('🗑️ 销毁文流助手...');
    
    try {
      // 清理平台插件
      wenliuPlatformRegistry.clear();
      
      // 清理状态
      this.state.clear();
      
      // 清理事件
      wenliuEventBus.clear();
  ...
  async handleFillContent(data) {
    try {
      ...
      // 使用内容处理服务处理数据
      const contentService = window.wenliuContentService;
...
// 全局应用实例
window.wenliuApp = new wenliuApp();