...
class wenliuContentService {
  ...
  async fetchArticleDetail(articleId) {
    const response = await wenliuApiService.articles.get(articleId, 'inline');
    if (!response.success) {
  ...
  async convertArticleFormat(content, targetFormat, style = 'default') {
    const response = await wenliuApiService.content.convert(content || '', targetFormat, style);
  ...
  async fetchArticleMarkdown(articleId) {
    const response = await wenliuApiService.articles.get(articleId, 'raw');
    if (!response.success) {
...
// 全局内容服务实例
window.wenliuContentService = new wenliuContentService();