...
class UtilsService {
  ...
  showNotification(message, type = 'info', duration = 3000) {
    ...
    notification.className = `wenliu-notification wenliu-notification--${type}`;
    ...
    // 添加动画样式
    if (!document.querySelector('#wenliu-notification-styles')) {
      const style = document.createElement('style');
      style.id = 'wenliu-notification-styles';
      style.textContent = `
  ...
  showLoadingOverlay(element) {
    ...
    overlay.className = 'wenliu-loading-overlay';
    ...
    // 添加旋转动画
    if (!document.querySelector('#wenliu-spinner-styles')) {
      const style = document.createElement('style');
  ...
  isWeChatEditorPage() {
    ...
    if (window.wenliuPlatformManager) {
      const platform = window.wenliuPlatformManager.findPlatformByUrl(window.location.href);
...
// 创建全局实例
window.wenliuUtilsService = new UtilsService();

console.log('✅ 文流工具服务已加载');