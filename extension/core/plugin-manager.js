...
class PluginManager {
  ...
  register(plugin) {
    ...
    wenliuEventBus.emit('plugin:registered', plugin);
  }
  ...
  async loadPlugins(pluginConfigs) {
    ...
    try {
      ...
      wenliuEventBus.emit('plugins:loaded', { successCount, failureCount });
  ...
  async loadPlugin(config) {
    ...
    try {
      ...
      wenliuEventBus.emit('plugin:loaded', { id, instance: plugin.instance });
      ...
      wenliuEventBus.emit('plugin:error', { id, error });
      throw error;
    }
  }
  ...
  async unloadPlugin(id) {
    ...
    try {
      ...
      wenliuEventBus.emit('plugin:unloaded', { id });
...
// 全局插件管理器实例
window.wenliuPluginManager = new PluginManager();