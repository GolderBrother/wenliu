...
class wenliuFeatures {
  ...
  bindEvents() {
    // 监听面板显示事件
    wenliuEventBus.on('panel:show', () => {
      this.onPanelShow();
    });

    // 监听预设加载完成事件
    wenliuEventBus.on('presets:loaded', (data) => {
      this.onPresetsLoaded(data);
    });

    // 监听预设切换事件
    wenliuEventBus.on('presets:changed', (data) => {
      this.onPresetChanged(data);
    });

    // 监听应用就绪事件
    wenliuEventBus.on('app:ready', () => {
      this.onAppReady();
    });
  ...
  showLoginForm() {
    const content = `
      <div style="text-align: center; padding: 20px;">
        ...
        <button id="wenliu-login-btn" style="
    ...
    // 绑定登录按钮事件
    const loginBtn = document.getElementById('wenliu-login-btn');
    if (loginBtn) {
      loginBtn.addEventListener('click', async () => {
        try {
          const response = await this.getwenliuUrls();
  ...
  createArticleItem(article) {
    ...
    // 获取当前平台信息和按钮配置
    const currentPlatform = window.wenliuApp?.getCurrentPlatform();
    ...
    const fillButtonHtml = `
      <button class="wenliu-fill-btn" ${fillDisabled ? 'disabled' : ''} style="
    ...
    const copyButtonHtml = noCopyButton ? '' : `
      <button class="wenliu-copy-btn" style="
    ...
    return `
      <div class="wenliu-article-item" style="
      ...
      " data-article-id="${article.id}">
        <div style="font-weight: 500; font-size: 13px; color: #2d3436; margin-bottom: 6px; display: flex; align-items: center; justify-content: space-between;">
          ...
          <button class="wenliu-edit-btn" style="
            background: none;
  ...
  createPagination() {
    ...
    return `
      ...
      ">
        <button id="wenliu-prev-page" ${prevDisabled ? 'disabled' : ''} style="
        ...
        <button id="wenliu-next-page" ${nextDisabled ? 'disabled' : ''} style="
  ...
  bindArticleEvents() {
    // 填充按钮事件
    document.querySelectorAll('.wenliu-fill-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const articleId = btn.closest('.wenliu-article-item').dataset.articleId;
        this.fillArticle(articleId, btn);
      });
    });

    // 复制按钮事件
    document.querySelectorAll('.wenliu-copy-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const articleId = btn.closest('.wenliu-article-item').dataset.articleId;
    ...
    // 编辑按钮事件
    document.querySelectorAll('.wenliu-edit-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const articleId = btn.closest('.wenliu-article-item').dataset.articleId;
    ...
    // 分页事件
    const prevBtn = document.getElementById('wenliu-prev-page');
    const nextBtn = document.getElementById('wenliu-next-page');
  ...
  async fillArticle(articleId, buttonElement) {
    ...
    try {
      ...
      // 使用App的完整填充功能
      const result = await window.wenliuApp.handleFillContent({ 
        articleId: articleId,
  ...
  updatePresetSelector() {
    const presetSelector = document.getElementById('wenliu-preset-selector');
    if (!presetSelector) {
  ...
  onPresetSelectorChange(event) {
    ...
    if (presetId === 'none') {
      // 选择不使用预设
      if (window.wenliuApp) {
        window.wenliuApp.setSelectedPreset(null);
        console.log('🎯 用户选择不使用预设');
      }
    } else if (presetId && window.wenliuApp) {
      window.wenliuApp.setSelectedPreset(presetId);
  ...
  async editArticle(articleId) {
    try {
      console.log('跳转到文流编辑页面:', articleId);
      
      // 通过background获取配置URL
      const response = await this.getwenliuUrls(articleId);
      const editorUrl = response.data.editorUrl;
      
      // 在新标签页中打开编辑页面
      window.open(editorUrl, '_blank');
      
      this.showToast('已跳转到文流编辑页面', 'success');
  ...
  showLoading(message = '加载中...') {
    ...
    // 添加旋转动画
    if (!document.getElementById('wenliu-spinner-styles')) {
      const style = document.createElement('style');
      style.id = 'wenliu-spinner-styles';
      style.textContent = `
        @keyframes spin {
  ...
  showError(message) {
    const content = `
      <div style="text-align: center; padding: 40px 20px;">
        ...
        <button onclick="window.wenliuFeatures.checkLoginStatus()" style="
          background: #667eea;
          border: none;
          color: white;
          padding: 8px 16px;
  ...
  updatePanelContent(content) {
    if (window.wenliuPanel) {
      window.wenliuPanel.updateContent(content);
    }
  }
   ...
   * 获取文流配置URL
   */
  async getwenliuUrls(articleId = null) {
    return new Promise((resolve, reject) => {
      chrome.runtime.sendMessage({
        action: 'getwenliuUrls',
        data: { articleId }
      }, (response) => {
...
// 全局实例
window.wenliuFeatures = new wenliuFeatures();